...
- (id)toObject
{
	...
	switch(desc) {
		...
		case 'bool':
			...
			break;
		default:
			NSLog(@"Unknown AEType: '%C%C%C%C'", _cmd, desc >> 24 & 0xFF, desc >> 16 & 0xFF, desc >> 8 & 0xFF, desc & 0xFF);
			ret = [self stringValue];
	}
	return ret;
}
	...
	for (i=1; i <= [self numberOfItems]; i++) {
		...
		else // These are the keys that are based on keywords.  They can just be tossed in.
		{
			...
			switch (keyword) {
				...
				default: 
					key = [NSString stringWithFormat:@"%C%C%C%C", keyword >> 24 & 0xFF, keyword >> 16 & 0xFF, keyword >> 8 & 0xFF, keyword & 0xFF];
